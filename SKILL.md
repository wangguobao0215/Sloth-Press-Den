...
author: 树懒老K（拙一）
license: MIT
trigger: 用户说"写书"、"写电子书"、"出书"、"开始写书"、"排版"、"出版"、"出PDF"、"转PDF"、"出书稿"、"整本书"、"畅销书"、"写一本商业书"、"出畅销书"、"技术实操书"、"技术书"、"技术实操手册"、"技术手册"、"写一本传记"、"人物传记"、"非虚构写作"、"传记书"
metadata:
  hermes:
...
> <p align="center"><img src="https://raw.githubusercontent.com/wangguobao0215/Sloth-Press-Den/main/assets/qrcode.jpg" width="80" /></p>
>
> **匠书·出版排版引擎 + 写作工坊 = 从零到成书的完整流水线**
>
> 扫码关注「树懒老K」公众号，获取更多 AI 原生实践内容。
>
> *慢一点，深一度。*
...
**副标题原则**：副标题应补充主标题未覆盖的信息维度（领域/方法/交付物），**不能是主标题的同义转述，也不能重复书名中已有的关键词**。例如主标题含"本体"则副标题不应再出现"本体"。检查方法：去掉副标题后，书名是否丢失了关键交付信息？如果是，副标题合格。反例自查：如果副标题里出现了书名主词的同义词或近义词，换一个
...
- ✅ **书内技术架构图规范**：分层架构图/系统图用**雾霁蓝手工SVG**（配色/组件/布局模板/检查清单见 `references/book-diagrams.md`，借鉴 architecture-diagram 视觉语言但自建雾霁蓝体系，暖金仅作消费层点缀）；流程/时序/状态图用 **Mermaid**（工坊构建自动渲染）；每章概念插图用 `references/book-illustration-workflow.md`。不要在正文写 ASCII 框线图（`┌│└`），书稿质感撑不住
...
- Chrome 原生打印方案：`references/chrome-print-cjk.md`（去掉 Paged.js 后的实测要点，含10项关键修复）
  ...
  - 书内技术架构图：`references/book-diagrams.md`（雾霁蓝SVG设计系统 + Mermaid选型分工 + 三种布局模板 + 图注编号 + 检查清单）
  - 人物传记创作参考：`references/biography-writing.md`（基于《哈萨比斯：谷歌AI之脑》实战验证）
